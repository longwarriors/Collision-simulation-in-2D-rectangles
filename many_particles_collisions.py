...
radius = 0.02  # 圆盘半径
speed = 28 # 粒子运动速率
bound_l, bound_r = -0.5, 0.5  # 包围盒左右边界
bound_d, bound_u = -0.5, 0.5  # 包围盒上下边界
n_particles = 100  # 粒子数量
n_increments = 10000  # dicrete time increments
t_end = 5  # 秒
dt = t_end / n_increments
t_nodes = torch.linspace(
    start=0,
    end=t_end,
    steps=n_increments + 1,
    device=engine)  # my convention
...
for n in range(n_increments):
    ...
    """
    collisions_vertical = ((coords[n, :, 0] < bound_l + radius) & (velocs[n, :, 0] < 0)) | ((coords[n, :, 0] > bound_r - radius) & (velocs[n, :, 0] > 0))
    collisions_horizon  = ((coords[n, :, 1] < bound_d + radius) & (velocs[n, :, 1] < 0)) | ((coords[n, :, 1] > bound_u - radius) & (velocs[n, :, 1] > 0))
    ...
    """相互碰撞后的粒子速度"""
    k_collided_pairs = dr_collided_pairs * dv_collided_pairs # 如果没有碰撞则size=(0, 2)
    k_collided_pairs = k_collided_pairs.sum(dim=1)  # 内积变为vector
...
ani = animation.FuncAnimation(
    fig, update,
    frames=np.arange(0, n_increments + 1, 20, dtype=int),
